    ...
    if (savedLayoutString) {
      try {
        ...
        if (parsedLayout.positions && typeof parsedLayout.zoomLevel === 'number' && typeof parsedLayout.viewBox === 'string') {
            ...
            console.log("Loaded full layout from localStorage.");
    ...
    <div className="flex flex-col items-center w-full h-screen p-4 box-border">
      ...
      <div className="mb-4 flex items-center gap-4 flex-wrap justify-center w-full">
        <div>
          ...
          <select
            id="speaker-filter"
            className="px-2 py-1 border border-gray-300 rounded bg-white text-gray-800"
        ...
        <div className="flex items-center gap-2">
         ...
         <div className="flex items-center gap-2">
            ...
            <button className="p-2 bg-gray-100 hover:bg-gray-200 rounded-full" onClick={decreaseFontSize} title="Decrease Font Size">
                <MinusSquare size={20} />
            </button>
            <button className="p-2 bg-gray-100 hover:bg-gray-200 rounded-full" onClick={increaseFontSize} title="Increase Font Size">
                <PlusSquare size={20} />
            </button>
      ...
      <div className="border border-gray-300 rounded-lg overflow-hidden bg-white p-4 w-full flex-grow" style={{ minHeight: "500px" }}>
        ...
        <svg
          ...
          style={{ cursor: isPanning ? 'grabbing' : (isEditMode && draggingConcept ? 'grabbing' : (isEditMode ? 'grab' : 'default')) }}
                ...
                return (
                  <g
                    ...
                    onClick={(e) => {
                        ...
                        if (expanderClicked) {
                            if (hasChildren) { e.stopPropagation(); toggleExpand(concept.id); }
                        } else if (hasChildren && !isEditMode) {
                    ...
                    onMouseDown={(e) => {
                      if (isEditMode && !isRoot) {
                        if(e.target instanceof SVGRectElement){ 
                             startDragConcept(e, concept.id);
                        }
                      }
                    }}
                    style={{
                      cursor: (hasChildren && !isEditMode) ? 'pointer' :
                              (isRoot ? 'default' :
                              (isEditMode ? (isDraggingThis ? 'grabbing' : 'grab') : 'default'))
                    }}
                  >
                    <rect
                    ...
                    >
                      <div
                        style={{
                          ...
                          fontWeight: isRoot ? 'bold' : 'normal', textAlign: 'center',
                    ...
                    {hasChildren && (
                      <g data-expander="true" style={{ cursor: 'pointer' }} >
                        <circle
                          cx={concept.x + concept.width / 2 - 10} cy={concept.y + concept.height / 2 - 10}
                          r={7} fill="white" opacity={opacity}
                        />
                        <text
                          x={concept.x + concept.width / 2 - 10} y={concept.y + concept.height / 2 - 10}
                          textAnchor="middle" dominantBaseline="central"
                          fill={speakerColor} 
                          fontSize={Math.max(6, 12 * fontSizeScale)} 
                          fontWeight="bold"
                          opacity={opacity} pointerEvents="none"
                        >
                          {isExpanded ? "-" : "+"}
      ...
      <div className="mt-4 flex flex-wrap gap-2 justify-center">
        {Object.entries(speakers).map(([name, color]) => (
          <div key={name} className="flex items-center gap-1"><div className="w-4 h-4 rounded-full" style={{ backgroundColor: color }}></div><span className="text-sm text-gray-700">{name}</span></div>
        ))}
      </div>
      <div className="mt-4 text-sm text-gray-600 text-center max-w-lg">
        <p>
          Default video loaded. Use timeline/player to scrub.
          {isEditMode ? " In Edit Mode, arrange map, then 'Save Layout'." : " Click concepts (+/-) to expand/collapse."}
        </p>
